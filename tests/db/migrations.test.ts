...
describe("migrations: structural facts about moni_test", () => {
  it("creates exactly the 35 application tables, including four long-term-savings tables", async () => {
    ...
    expect(Number(rows[0].count)).toBe(35);
  });

  it("enables + forces RLS on exactly 34 tables (every user-owned table, excluding fx_rates)", async () => {
    const { rows } = await elevatedPool.query<{ count: string }>(
      `select count(*)::int as count
       from pg_class c
    ...
    expect(Number(rows[0].count)).toBe(34);
  });

  it("has a tenant-isolation policy on every one of those 34 tables", async () => {
