...
describe("migrations: structural facts about moni_test", () => {
  it("creates exactly the 52 application tables, including the investment activity model", async () => {
  ...
  it("enables + forces RLS on exactly 51 tables (every user-owned and group-owned table, excluding fx_rates)", async () => {
    const { rows } = await elevatedPool.query<{ count: string }>(
      `select count(*)::int as count
       from pg_class c
    ...
    expect(Number(rows[0].count)).toBe(51);
  });

  it("has a tenant-isolation policy on every one of those 51 tables", async () => {
