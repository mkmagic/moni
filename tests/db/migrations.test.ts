...
describe("migrations: structural facts about moni_test", () => {
  it("creates exactly the 27 application tables, including seven investment tables", async () => {
  ...
  it("enables + forces RLS on exactly 26 tables (every user-owned table, excluding fx_rates)", async () => {
    const { rows } = await elevatedPool.query<{ count: string }>(
      `select count(*)::int as count
       from pg_class c
  ...
  it("has a tenant-isolation policy on every one of those 26 tables", async () => {
