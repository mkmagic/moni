...
describe("migrations: structural facts about moni_test", () => {
  it("creates exactly the 53 application tables, including the investment activity model", async () => {
  ...
  it("enables + forces RLS on exactly 52 tables (every user-owned and group-owned table, excluding fx_rates)", async () => {
    const { rows } = await elevatedPool.query<{ count: string }>(
      `select count(*)::int as count
       from pg_class c
  ...
  it("has a tenant-isolation policy on every one of those 52 tables", async () => {
