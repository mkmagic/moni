...
- **The net-worth sparkline now starts at the month the user joined.** A new household links accounts that have existed for years in their first month, so the stock history read ~0 for every pre-join month and then spiked into the join month — a jump that says "when you signed up", not "how your money moved". `getOverview` now reads `users.createdAt`, and the returned `netWorthHistory` is filtered to `month >= joinMonth`. The trim is applied to the **displayed** array only, after `netWorthTrend`'s baseline is taken from the full six-month window — so a recent joiner simply gets a null trend (zero baseline) rather than a wrong "6mo" badge, and an established user is unaffected.
- **Verified in the browser (headless Chromium, `dana@moni.demo`).** The demo seed already reproduces the spike: Dana's account snapshots and investment all land in late July, so before that her net worth reads ~0. Spreading the three snapshot dates across May–July and toggling only her `created_at` gives a faithful before/after of old-code (no trim → flat-near-zero months then a spike) vs new-code (trim → the line starts at the join month with a real value). Confirmed the Expenses tile shows the figure with **no** trend icon and the "All caught up" panel has **no** spending row.
- **Test bookkeeping worth knowing:** `tests/db/dashboard-*` seed users via `createUser`, which stamps `created_at = now`, so the join-month trim would erase the six-month history those tests assert. Both fixtures now backdate `created_at` to 2000 (the trim has its own dedicated test that sets a recent join month and checks the history starts there). The three `expenseTrend` DB tests were removed with the feature.

### 2026-08-22 — agent token management UI (#113 Phase 5): a settings surface with a one-time secret and inline confirms

- **The MCP token surface lives on Settings › AI & MCP, below Smart Categorize.** The tab already existed (`settings-tabs.tsx`, "AI & MCP"). Two amber-on switches now share that view (Smart Categorize + Agent access) and the mint button is amber `primary` — more amber-in-one-view than the per-view-accent rule likes, but it's the honest shape of a settings page with two boolean prefs plus one primary action, and each is its own Card. Built mobile-first; **flag for the owner if two switches + a primary button read as busy.**
- **A one-time secret is revealed in place, not in a modal.** Mint/rotate return the secret in the POST response (shown once, only a hash is stored); the reveal is an amber-tinted Card at the top of the panel with the `moni_agent_…` string in a `font-mono break-all` box, a Copy button (`navigator.clipboard`, wrapped in try/catch — the string stays selectable if the clipboard is blocked), and a Done dismiss. No `dialog.tsx` needed, and it dodges the alert/confirm-dialog trap that freezes the extension.
- **Destructive/semi-destructive actions use an inline two-step confirm, not `window.confirm`.** Revoke and Rotate each swap the row's buttons for "Revoke/Rotate this token? [Yes] [Cancel]" via a per-row `confirm` state. `Yes` is `variant="destructive"` for revoke, `primary` for rotate. This keeps everything on-page (no modal, no browser dialog) and matches the delete-account precedent of naming the action rather than asking "are you sure?".
- **Dates are formatted server-side and passed as strings** (`Intl.DateTimeFormat("en-GB", {dateStyle:"medium"})` in the server component), so the client panel holds zero date logic — the 2026-07-26 hydration rule. Status (Active/Revoked/Expired) is a `Badge` tinted `positive`/`negative`; a dead token is struck through with its actions removed.
- **A router.refresh() after every mutation re-reads the server component**, which re-lists tokens and re-formats their dates — so the client never recomputes a list it was handed. Opt-in toggle also refreshes, which is what makes the token panel appear/disappear.
- **Verified end to end in the browser at a 500px viewport** (which doubles as the narrow/mobile pass the recurring `resize_window` limitation otherwise blocks): opt-in switch → amber-on + panel appears → mint → secret reveal → token row with Active badge + Rotate/Revoke → inline revoke confirm. The dev DB carried a pre-existing revoked `curl-check` token from the Phase-1/2 live checks, which conveniently exercised the struck-through Revoked row. **A wider desktop pass (≥1024px) was not taken from here — please eyeball the two-switch density there.**

- **The old toolbar's six controls-at-once became a tap-first default + an Advanced Search.** Default sections stack mobile-first: Search, Timeframe (All / This Week / This Month / Last 3 Months), Type (All / Income / Payment), Size (All / S / M / L), Category. Advanced Search is a local `useState` collapsible (no new `ui/` primitive — the ad-hoc pattern, per investments-screen) holding the scope toggle + explicit From/To + Min/Max. Verified in the browser at desktop width: sections render clean, pills select amber, Advanced expands with a rotating chevron and a focus ring, no console errors.
- **A prominent Income/Payment or Size filter over only the loaded 100-row window silently under-counts — Codex's adversarial review flagged it and the owner chose a toggle, not an either/or.** Default stays client-side (fast); the Advanced "Search entire history" `Switch` sets `scope=all`, and the domain (`listEntries`) then applies Income/Payment and size **after decryption, before the window cap** — complete. Type & Size live in the URL (one source of truth for both the client predicate and the server filter); Search & amount stay client `controls`. A `tests/db` test pins the completeness (income older than the display page is still found).
- **Timeframe presets must set `from` AND `to` in ONE navigation.** The old single-key `setServerFilter` cloned the same `useSearchParams` snapshot per call, so two sequential calls lost an endpoint. Replaced with one atomic `setUrl(patch)`. Verified live: clicking Payment then Last-3-Months produced `?direction=payment&from=2026-05-17&to=2026-08-17` — the direction survived, both dates set together.
- **Income/Payment is Moni's flow definition, not raw sign.** `matchesDirection` (shared `lib/transactions/predicates.ts`) drops `excluded` and `isTransfer` rows before reading the sign, so a credit-card settlement isn't mislabelled a huge payment — same rule as `flows.ts`. Size is a `Decimal` magnitude band (S<₪100 / M ₪100–1,000 / L>₪1,000), disjoint boundaries, and **excludes pending-FX rows** whose amount is still in the entered currency.
- **Timeframe uses Asia/Jerusalem "today", computed server-side and passed down** (reused `israelDate`), never the browser clock — the hydration rule, and it agrees with the existing `israelWeekStart` (Sunday) convention.
- **Advanced Search auto-opens when it holds an active value the default sections can't show** (a deep-linked custom range, a min/max, or `scope=all`), so a hidden filter can't silently narrow the table.
- **Smart Categorize's mobile cramping was the `flex flex-col items-end` wrapper.** Now `w-full sm:w-auto`, dropping to its own full-width row on a phone. Promoted `PillButton` out of `backfill-window-picker.tsx` into `src/components/pill-button.tsx` (one pill idiom, bumped `min-h` for touch).
- **Two amber cautions for the owner to eyeball.** (1) Several selected pills (10% amber tint) can show at once alongside the Advanced scope `Switch` (amber track) — all accents, no fill, but it's more amber-in-one-view than prior screens; flag if it reads busy. (2) **The true mobile (narrow-viewport) pass is unverified from here** — `resize_window` still does not reflow the content viewport (the recurring limitation), so the layout was built mobile-first with explicit responsive classes and confirmed only at desktop width + via a clean production build. Please give it a phone-width look.
