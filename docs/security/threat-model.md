...
#### 5.6.1 The agent token
- **Opaque, revocable, hashed at rest.** The token is a random opaque secret, stored server-side only as a hash (an `agent_tokens` row: `user_id`, token hash, the wrapped-DK sealed to the token at mint, `expires_at`, `last_used_at`, `revoked_at`), RLS-scoped to its user. Not a JWT, not OAuth — for a single self-hosted box, **server-side revocation (kill the row) is the primary control**; instant, no crypto-expiry games. `expires_at` (TTL) is a backstop, not the main lever. OAuth is deferred: Moni would be its own IdP, it is heavy for one box, and real MCP-client OAuth support is uneven.
...
- **Management + observability.** Users get a token-management surface (list active tokens, `last_used_at`, revoke/rotate). Every token-authenticated tool call is audit-logged (user, tool, argument shape, row count, timestamp — **never plaintext**), because a standing remote read with no log is how a stolen token goes unnoticed.
...
- **Volume caps + audit, not row caps.** Because the token owner has accepted full-DB read reach, the check on runaway exfiltration is **per-token volume/rate limits** and an **access audit log** (§5.6.1) that makes an anomalous whole-ledger sweep visible after the fact — not an artificial cap on how much the user's own agent may read.
