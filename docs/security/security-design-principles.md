...
25. **Headless/MCP reads use a per-user, DK-only, per-request key — opt-in, and never CK.** A user may opt **their own** account into AI reads by minting a **per-user agent token** that lets the server unwrap **that user's DK for the duration of one request** (decrypt, aggregate, wipe — no standing warm key). The token is a **DK-only, read-only, single-user** capability: it can never arm the credential window, reach CK/bank credentials, write, or cross to another user. Accepted trade the opting-in user makes: a stolen token, or a prompt-injected model, can **disclose that user's own Tier-1 data**. Mitigated by opaque hashed-at-rest tokens, instant server-side revocation (primary) + TTL backstop, per-token volume/rate caps, and an access audit log. A user who does not opt in loses nothing else. (§5.6, `../design/mcp-and-api.md`)
...
- **Does any MCP/agent read decrypt for a user who has not opted in, or reach CK?** It must not — decryption needs that user's own opt-in agent token, is DK-only and per-request, and no agent path may arm the credential window or unwrap CK.
