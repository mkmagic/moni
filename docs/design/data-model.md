...
### Budget
- **`budget_ceilings`** — a monthly target per category, effective-dated. `owner_id`, `category_id` (composite FK), `amount_ct` (Tier-1 → encrypted, **nullable**), `effective_from` (that month's first day), `rollover`, `version`. Editing writes a new row from that month forward and leaves the old one, so a finished month keeps the number that was actually in force in it. A **null `category_id`** is the residual ceiling — "everything else", covering all spending no other ceiling reaches — and its uniqueness rests on a `NULLS NOT DISTINCT` index, since Postgres otherwise treats every NULL as distinct. A **null `amount_ct`** ends the line from that month forward, which is how a budget line stops without erasing the months it governed. `category_id`, `effective_from` and `rollover` stay plaintext so SQL narrows before anything is decrypted. **One ceiling per branch** — a parent group or its children, never both — is enforced in the domain layer, not the schema: it spans rows the database cannot compare, and the effective-dating means several rows per category are correct. See [ADR 0010](../adr/0010-a-budget-is-a-ceiling-and-a-ceiling-is-effective-dated.md).
- **`budget_incomes`** — planned monthly income, one effective-dated figure per user. Same shape and same reasoning; deliberately not per-category.
