...
- **Opaque access tokens without a server key.** `moni_oauth_at_…` is a prefix plus a random 32-byte secret — nothing more. The wrapped DK it opens lives server-side on the grant row (`access_wrapped_dk`), never in the token, so a token holder cannot reconstruct DK offline or keep it past expiry/revocation (an earlier design packed the wrap into the token itself — a standing key that broke threat-model §5.6). Each request looks the grant up by the secret's SHA-256 hash and enforces `access_expires_at`, `revoked_at`, RFC 8707 audience (`resource`), and the user's opt-in before unwrapping DK. `moni_oauth_rt_…` is likewise hash-looked-up and rotates atomically on every refresh; a replay of a rotated-away refresh token is detected via `previous_refresh_token_hash` and revokes the whole grant family (RFC 9700 §4.14). A refresh token is only issued when `offline_access` was granted.
