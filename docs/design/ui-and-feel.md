...
- **Budget headline** (`budget-screen.tsx` `Headline`) — the month's hero: spend-against-budget as one big figure over a full-width `BudgetBar`, with `N days left · on track for X` beside it. Two rules it exists to encode. The numerator is **budgeted** spend, never `spentTotal` — only spending a ceiling governs is comparable with `ceilingTotal`. And the bar carries **no pace marker**, for the same reason Fixed rows don't: the total includes rent, which is fully spent on the 1st by design. The pace signal lives in the projection instead, which extrapolates only the everyday half — scaling a lump-sum rent by the fraction of the month elapsed claims a ₪4,500 rent is heading for ₪34,875.
- **Multi-step wizard** (`budget-setup.tsx`) — one Card, a `Step N of M · <what>` heading, and a footer rule with Back on the left and the forward action on the right. Steps are split by the *kind* of question, not to shorten a form: confirming near-certain figures, making a judgement, and seeing the verdict are different acts. Nothing is written until the last step. A number the wizard proposes is rounded **at this edge** (the domain layer must not round) and rounded **up**, since a ceiling rounded down is one the user's own history already breaks.
